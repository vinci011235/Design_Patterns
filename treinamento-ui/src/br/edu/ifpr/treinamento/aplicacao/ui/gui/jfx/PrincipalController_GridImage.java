...
public class PrincipalController_GridImage {
   private static final String FXML_PATH_NAME =
                               "/br/edu/ifpr/treinamento/aplicacao/ui/jfx/";
   ...
   private static final KeyCombination CURSOS_KEY =
            new KeyCodeCombination(KeyCode.C,KeyCombination.CONTROL_DOWN);
   private static final KeyCombination MÓDULOS_KEY =
            new KeyCodeCombination(KeyCode.D,KeyCombination.CONTROL_DOWN);
   private static final KeyCombination MATRÍCULAS_KEY =
            new KeyCodeCombination(KeyCode.M,KeyCombination.CONTROL_DOWN);
   private static final KeyCombination ALUNOS_KEY =
            new KeyCodeCombination(KeyCode.A,KeyCombination.CONTROL_DOWN);
   private static final KeyCombination INSTRUTORES_KEY =
            new KeyCodeCombination(KeyCode.I,KeyCombination.CONTROL_DOWN);
   private static final KeyCombination RELATÓRIOS_KEY =
            new KeyCodeCombination(KeyCode.R,KeyCombination.CONTROL_DOWN);
   private static final KeyCombination SERVIÇOS_KEY =
   ...
   @FXML
   private void onImageViewLabelMouseClicked(MouseEvent ev)
                throws IOException {
	...
	private void onImageViewMouseEntered(MouseEvent ev) {
	   changeLabelStyle((Node) ev.getSource(),
...
//            root.centerProperty().set(new CadastroCurso(jpaService));
            ...
            scene.focusOwnerProperty().addListener(
                                   controller.getFocusPropertyChangeListener());
         }
         else
            if (obj == ivMódulos || obj == lbMódulos) {
               ...
               if (obj == ivMatriculas || obj == lbMatriculas) {
                  ;
               }
               else
                  if (obj == ivAlunos || obj == lbAlunos) {
                     ;
                  }
                  else
                     if (obj == ivInstrutores || obj == lbInstrutores) {
                        ;
                     }
                     else
                        if (obj == ivRelatorios || obj == lbRelatorios) {
                           ;
                        }
                        else
                           if (obj == ivServicos || obj == lbServicos) {
                              ;
                           }
         if (root == null) {
...
//      int  index  = (row / 2) * 3 + column;
   ...
   private int getLabelIndexFromGrid(Node node) {
      ...
      return (GridPane.getRowIndex(node) / 2) * 3 +
             GridPane.getColumnIndex(node);
   }

   private void initTooltips() {
      Tooltip ttpCursos      = new Tooltip(String.format("Cursos (%s)",
                                                  CURSOS_KEY.getDisplayText()));
      Tooltip ttpMódulos     = new Tooltip(String.format("Módulos (%s)",
                                                 MÓDULOS_KEY.getDisplayText()));
      Tooltip ttpMatriculas  = new Tooltip(String.format("Matrículas (%s)",
                                              MATRÍCULAS_KEY.getDisplayText()));
      Tooltip ttpAlunos      = new Tooltip(String.format("Alunos (%s)",
                                                  ALUNOS_KEY.getDisplayText()));
      Tooltip ttpInstrutores = new Tooltip(String.format("Instrutores (%s)",
                                             INSTRUTORES_KEY.getDisplayText()));
      Tooltip ttpRelatorios  = new Tooltip(String.format("Relatórios (%s)",
                                              RELATÓRIOS_KEY.getDisplayText()));
      Tooltip ttpServicos    = new Tooltip(String.format("Serviços (%s)",
                                                SERVIÇOS_KEY.getDisplayText()));
      Tooltip ttpSair        = new Tooltip(String.format("Sair (%s)",
                                                    SAIR_KEY.getDisplayText()));
   ...
   private class SceneKeyPressed implements EventHandler<KeyEvent> {
      ...
      public void handle(KeyEvent ev) {
         if (CURSOS_KEY.match(ev))
            try { changeView(lbCursos); }
            catch (IOException ex) { ex.printStackTrace(); }
         else
            if (MÓDULOS_KEY.match(ev))
