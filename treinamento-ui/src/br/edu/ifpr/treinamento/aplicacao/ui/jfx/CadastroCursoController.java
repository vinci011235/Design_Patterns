...
public class CadastroCursoController {
   ...
   private static final Logger LOGGER =
                      Logger.getLogger(CadastroCursoController.class.getName());
...
//                new ModuloFXBean("MÓDULO EEE",DateTimeUtils.localDateNowPlusDays(8)));
   ...
   private void toEntity() {
      ...
      curso.setInicio(DateTimeUtils.localDateToDate(
                                              fxbCurso.inicioProperty().get()));
      // Módulos do Curso
      ...
      for (ModuloFXBean mbean : fxbCurso.modulosProperty()) {
         ...
         m.setInicio(DateTimeUtils.localDateToDate(
                                                 mbean.inicioProperty().get()));
         // Instrutor do Módulo
         ...
         i.setNascimento(DateTimeUtils.localDateToDate(
                                             ibean.nascimentoProperty().get()));
   ...
   private void fromEntity() {
      ...
      fxbCurso.inicioProperty().set(
                              DateTimeUtils.dateToLocalDate(curso.getInicio()));
      fxbCurso.modulosProperty().clear();
      // Módulos do Curso
      for (Modulo m : curso.getModulos()) {
         ...
         mbean.inicioProperty().set(
                                  DateTimeUtils.dateToLocalDate(m.getInicio()));
         ...
         ibean.nascimentoProperty().set(
                              DateTimeUtils.dateToLocalDate(i.getNascimento()));
